...
La estructura debe ser idéntica a la que ya conoces.`;
    ...
    const claudeResponse = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: 'claude-haiku-4-5-20251001', // Claude Haiku 4.5 (NUEVO - disponible)
        max_tokens: 12000,
        messages: [
          {
            role: 'user',
    ...
    if (!claudeResponse.ok) {
      ...
      throw new Error(`Claude API error (${claudeResponse.status}): ${errorMsg}`);
    }
    ...
    if (!claudeData.content || !Array.isArray(claudeData.content) || claudeData.content.length === 0) {
      throw new Error('Claude devolvió una respuesta vacía. Intenta nuevamente.');
    }

    let jsonText = claudeData.content.map(b => b.text || '').join('');
    
    if (!jsonText || jsonText.trim() === '') {
      throw new Error('Claude devolvió contenido vacío. Intenta nuevamente.');
    }

    // DEBUG: Log para ver qué responde Claude
    console.log('=== CLAUDE RESPONSE ===');
    console.log('Status:', claudeResponse.status);
    console.log('Raw text (first 500 chars):', jsonText.substring(0, 500));
    console.log('=====================');
    ...
    try {
      ...
      throw new Error(`No se pudo parsear la respuesta de Claude como JSON. Primeros 200 caracteres: ${jsonText.substring(0, 200)}...`);
    }
